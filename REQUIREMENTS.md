...
- 每個來源 repo 遵循自己的 `CLAUDE.md` 資料憲法（Fail-Loud / SSOT / 分層）。
  > 📌 **2026-08-27 補註（新增指路，本行上方原句一字未動）**：本句陳述的是**來源** repo 的狀態，
  > 至今仍然成立。同日 user 頒布「AI 總管與虛擬軟體公司運行規範」後，本 repo 新建了
  > 根目錄 `CLAUDE.md`，**範圍嚴格限縮在流程治理**（誰動手、誰驗收、什麼時候可以問客戶），
  > **資料**憲法仍在上游三個來源 repo，本 repo 不另立資料憲法。
  > 讀到本句時請勿據以推論「本 repo 沒有 `CLAUDE.md`」。處置理由見 `CLAUDE.md` §0 與 §4。
