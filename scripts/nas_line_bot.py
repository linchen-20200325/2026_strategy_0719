...
--------------------------------------
    ...
    SUBSCRIBERS_FILE            訂閱清單路徑，預設 subscribers.json（與 run_pipeline --subscribers 對齊）
...
# ── 授權名單（本機 JSON 持久化；管理員用 LINE 加人免重啟）─────────────────────
class JsonAllowStore:
    """{ "allow": [ {"id","name"} ] } 本機檔。空名單 + 空 env = 不限制（對外開放）。"""
    ...
    def _load(self) -> dict:
        ...
        try:
            ...
            logger.warning("讀授權檔失敗 %s：%s → 視為空名單", self.path, exc)
            return {"allow": []}
    ...
    def ids(self) -> set[str]:
        return {str(a.get("id")) for a in self._load().get("allow", []) if a.get("id")}
    ...
    def grant(self, user_id: str, name: str = "") -> tuple[bool, str]:
        ...
        data["allow"].append({"id": uid, "name": (name or "").strip()})
        self._save(data)
    ...
    def revoke(self, user_id: str) -> tuple[bool, str]:
        uid = (user_id or "").strip()
        data = self._load()
        before = len(data["allow"])
        data["allow"] = [a for a in data["allow"] if str(a.get("id")) != uid]
        if len(data["allow"]) == before:
            return False, f"{uid[:8]}… 不在授權名單內。"
        self._save(data)
        return True, f"🗑️ 已撤銷 {uid[:8]}…"

    def format(self) -> str:
        lst = self._load().get("allow", [])
        if not lst:
...
def effective_allowed(allow_store: JsonAllowStore) -> set[str]:
    """有效授權集合 = 本機名單 ∪ env STRATEGY_ALLOW_USER（bootstrap）。"""
    return allow_store.ids() | _env_set("STRATEGY_ALLOW_USER")


def is_user_allowed(allow_store: JsonAllowStore, user_id: str) -> bool:
    """可用『加/刪/清單』者;有效集合為空 = 不限制（對外開放）。"""
    eff = effective_allowed(allow_store)
...
def handle_text(
    ...
    *,
    ...
    # 管理員指令：授權 / 撤銷 / 名單（寫本機 bot_allow.json，即時生效免重啟）
    ...
    if admin_action in ("grant", "revoke", "allowlist"):
        ...
        if admin_action == "allowlist":
            return allow_store.format()
        if admin_action == "grant":
            uid, name = _split_id_name(admin_arg)
            _, msg = allow_store.grant(uid, name)
        else:
            _, msg = allow_store.revoke(admin_arg.strip())
        return msg + "\n\n" + allow_store.format()

    # 一般指令（加/刪/清單）：需被授權
    if not is_user_allowed(allow_store, user_id):
        return ("你還沒被授權使用 🙅\n"
...
def _allow_store() -> JsonAllowStore:
    return JsonAllowStore(os.environ.get("STRATEGY_ALLOW_FILE", "bot_allow.json"))
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            return
        store, allow_store = _store(), _allow_store()
        for ev in events:
            if ev.get("type") != "message" or ev.get("message", {}).get("type") != "text":
                continue
            user_id = ev.get("source", {}).get("userId", "")
            text = ev.get("message", {}).get("text", "")
            logger.info("收到訊息 userId=%s：%r", user_id, text)
            try:
                reply = handle_text(text, user_id, store=store, allow_store=allow_store)
